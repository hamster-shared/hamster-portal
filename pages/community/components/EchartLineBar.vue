...
//月份数值
const monthlyGrowthData = ref([ 0, 15175, 9911, 7854, 9509, 9723, 9856, 6991, 6442, 3932, 8875, 13165, 11696, 13010, 16297, 15475, 13356, 15212, 11390, 12535, 7915, 4814, 3771, 4300, 2863 ,2400 , 3600, 79117,96538,77264,126491,60715,61012,50561]) 
...
const setGrowthData = () => {
  ...
  const curMonth = timeOne.getMonth();
  // const month = timeOne.getMonth(); //不显示当前月份数据
  const month = 1; //数组需要手动赋值，同时赋值月份，当前数据对应的月份
  ...
  if (year === baseYear.value && month > baseMonth.value || year > baseYear.value) {
    ...
    setTotalNumber();
    let tempMonth = (year - baseYear.value) * 12 + (curMonth - baseMonth.value);
