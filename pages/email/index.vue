<template>
  <div :class="[$device.isMobile ? 'email-top-bg-phone' : 'email-top-bg']">
    <div class="container px-5 mx-auto md:h-screen md:flex md:items-center">
        ...
        <div class="md:flex md:px-[5%] md:items-center">
          <div class="md:w-[45%] flex items-center">
            <div>
              <div class="text-center md:text-left">
                <div
                  class="mt-[10px] mb-[20px] text-[24px] md:text-[48px] leading-[36px] md:leading-[60px] font-extrabold font-family-extraBold text-[#FFFFFF]">

                  <label v-if="route.query.show == 'register'">Get notified as soon as the Blast mainnet
                    launches</label>
          ...
          <div class="md:w-[45%] flex justify-end">
            <div class="mt-[50px] mb-[60px] md:mb-0 md:mt-0">
              ...
              <div
                class="rounded-[13px] bg-[#FFFFFF] px-[30px] md:px-[40px] py-[10px]  text-[#151210] text-[14px] font-family-regular">
                <div class="form-label"><span class="text-[#FF4A4A] mr-2">*</span>Your Name</div>
                <input @change="checkFormData('contactName')" type="text" name="contactName"
                  v-model="formData.contactName" placeholder="Please enter Your Name" autocomplete="off" />
                <div class="form-label"><span class="text-[#FF4A4A] mr-2">*</span>Email Address</div>
                <input :class="{ 'input-error': emailErr }" @change="checkFormData('contactEmailAddress')" type="text"
                  name="contactEmailAddress" v-model="formData.contactEmailAddress"
                ...
                <div class="flex">
                  <div class="relative !w-[40%]">
                    <div class="select-div !rounded-r-none" :class="{ 'select-active': showOptions.contactPlatform }"
                      @click="showOptions.contactPlatform = !showOptions.contactPlatform">{{ optionLabel.contactPlatform
                      || optionsSocial[0]}}
                      <img src="~/assets/images/select-down.svg" class="w-[15px] up-tran" />
                    </div>
                    <div class="option-div" v-if="showOptions.contactPlatform" @touchstart="optionEnter"
                      @touchend="optionLeave" @mouseleave="optionLeave" @mouseenter="optionEnter">
                      <div @click="setOptionVal(item, 'contactPlatform')" v-for="(item, index) in optionsSocial"
                        :key="index" :value="item" :class="{ 'option-active': item == formData.contactPlatform }">{{ item }}
                      </div>
                    </div>
                  </div>
                  <input @change="checkFormData('contactInformation')" class="!w-[60%] !rounded-l-none" type="text"
                    name="contactInformation" v-model="formData.contactInformation" placeholder="Your Account..."
                    autocomplete="off" />
                </div>
                <div class="form-label">Topic</div>
                <textarea name="middlewareInformation" maxlength="200" v-model="formData.topic" rows="2"
                  placeholder=""></textarea>

                <button class="btn-css my-[10px] w-full text-white" :disabled="isDisabled"
  ...
  <div class="modal-bg" v-if="isShowModal">
    <div class="modal-div py-[30px] md:px-[46px] px-[30px] md:w-[500px] w-[80%]">
      ...
      <div class="text-[14px]">Thank you for submitting the information.  Our team will contact you soon. Please pay
        attention to checking your email.</div>
...
onMounted(() => {
  ...
  if (route.query.show == 'topic') {
    formData.topic = 'Please raise any product questions,report problems or leave feedback here';
